...
export default function IntradayPredictions() {
  ...
  const columnDefs = [
    { field: 'target_time', headerName: 'target_time', sortable: true, filter: 'agTextColumnFilter', valueFormatter: params => { if (!params.value) return ''; const date = new Date(params.value); return date.toLocaleString(); } },
    ...
    { field: 'predicted_close', headerName: 'predicted_close', sortable: true, filter: 'agNumberColumnFilter', valueFormatter: params => params.value?.toFixed(2) },
    { field: 'predicted_trend', headerName: 'predicted_trend', sortable: true, filter: 'agNumberColumnFilter', valueFormatter: params => params.value?.toFixed(2), cellStyle: params => { if (params.value > 0.5) return { color: '#50F178' }; if (params.value < -0.5) return { color: '#FF5555' }; return { color: '#F1FA8C' }; } },
    { field: 'predicted_strength', headerName: 'predicted_strength', sortable: true, filter: 'agNumberColumnFilter', valueFormatter: params => params.value?.toFixed(2) },
    { field: 'predicted_range', headerName: 'predicted_range', sortable: true, filter: 'agNumberColumnFilter', valueFormatter: params => params.value?.toFixed(2) },
    ...
    { field: 'momentum', headerName: 'momentum', sortable: true, filter: 'agTextColumnFilter' },
    { field: 'predicted_high_touched', headerName: 'high_touched', sortable: true, filter: 'agTextColumnFilter', valueFormatter: params => params.value ? 'Yes' : 'No', cellStyle: params => params.value ? { color: '#50F178' } : {} },
    { field: 'predicted_low_touched', headerName: 'low_touched', sortable: true, filter: 'agTextColumnFilter', valueFormatter: params => params.value ? 'Yes' : 'No', cellStyle: params => params.value ? { color: '#50F178' } : {} }
  ];
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: '5px' }}>
            {timeframes.map(tf => (
              <button key={tf} onClick={() => setTimeframe(tf)} style={{ padding: '8px 16px', background: timeframe === tf ? '#50F178' : '#429356', color: timeframe === tf ? '#21222C' : 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: timeframe === tf ? 'bold' : 'normal' }}>
                {tf}
              </button>
            ))}
          </div>
          <button onClick={fetchData} disabled={loading} style={{ padding: '8px 16px', background: '#429356', color: 'white', border: 'none', borderRadius: '4px', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.6 : 1 }}>
